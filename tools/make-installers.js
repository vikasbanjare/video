...
const MAC_SH = [
  ...
  'rm -rf "$EXT/Pulse"',
  '# remove EVERY older copy that shares the same extension ID — earlier builds',
  '# installed as "CutPilot" (user AND system level); if one survives, CEP can',
  '# load IT instead of this new install and every fix looks like it did nothing.',
  'rm -rf "$EXT/CutPilot" "$EXT"/CutPilot-* 2>/dev/null',
  'rm -rf "/Library/Application Support/Adobe/CEP/extensions/CutPilot" "/Library/Application Support/Adobe/CEP/extensions/Pulse" 2>/dev/null || true',
  '# clear the CEP caches so Premiere reloads the new files, not cached old ones',
  'rm -rf "$HOME/Library/Caches/CSXS" "$HOME/Library/Caches/com.adobe.cep"* 2>/dev/null || true',
...
function buildWin() {
  ...
  const PS = [
    ...
    '  # remove older same-ID installs + the CEP cache so the new build actually loads',
    '  if (Test-Path (Join-Path $ext "CutPilot")) { Remove-Item (Join-Path $ext "CutPilot") -Recurse -Force }',
    '  Get-ChildItem (Join-Path $env:LOCALAPPDATA "Temp") -Filter "cep_cache*" -Directory -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue',
